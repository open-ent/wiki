...
export const Index = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // clear interval if all pages have finished content generation and return
      if (generatingContentPages?.length === 0) {
        clearInterval(interval);
        return;
      }
      // call React Query to refetch the wiki data to update the pages' AI content generation status
      refetch();
    }, 5000);
              ...
              <SortableTree
                ...
                renderNode={({ node, isChild }) => (
                  ...
                  >
                    ...
                    {/* Show loading icon if page is still being generated by AI. */}
                    {!isNodeContentReady(node) && (
                      <span>
                        <Loading isLoading />
                      </span>
                    )}
                    ...
                    {isNodeContentReady(node) && (
                      <span className="d-flex">
                        {/* Show Hide Icon if page is visible */}
                        {!node.isVisible && (
                          <IconHide
                            width="20"
                            height="20"
                        ...
                        {/* Show Create Children button if page is not a child and not readonly */}
                        {!isChild && !isOnlyRead && (
                          <button
